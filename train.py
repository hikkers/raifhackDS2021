...
def plot_clusters(data):
    ...
    ax.scatter(th_centroids[:,0], th_centroids[:,1], s=50, c='black', 
               marker="x")
# собираем train и тест
#path = "/content/drive/MyDrive/RAIF/data/"

train = prep(df)
test = prep(test)

# Подход:
# Выполняем кластеризацию на в рамках целого датасета. # Получаем train_clustered & test_clustered

train_clustered = get_clusters(train, 10)
test_clustered = test.copy()
mapping = dict(zip(train_clustered["region"], train_clustered["cluster"]))
train_clustered.drop("centroids", axis=1)
test_clustered["cluster"] = test["region"].map(mapping)

#Очищаем признаки
train_clustered["id"] = train_clustered["id"].str[4:].astype(int)
train_clustered["street"] = train_clustered["street"].str[1:].astype(int)
test_clustered["id"] = test_clustered["id"].str[4:].astype(int)
test_clustered["street"] = test_clustered["street"].str[1:].astype(int)
...
train_clustered.to_csv("train_clustered.csv")
test_clustered.to_csv("test_clustered.csv")
